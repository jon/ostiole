...
func TestMEMAPReadsConfiguredTargetWord(t *testing.T) {
	...
	if err := dp.WriteAP(t.Context(), 0, 0x00, 2); err != nil {
		t.Fatal(err)
	}
	if err := dp.WriteAP(t.Context(), 0, 0x04, wordAddr); err != nil {
		t.Fatal(err)
	}
	value, err := dp.ReadAP(t.Context(), 0, 0x0c)
	if err != nil {
		t.Fatal(err)
	}
	if value != wordData {
...
func TestMEMAPRejectsMalformedCSW(t *testing.T) {
	...
	if _, err := dp.ReadAP(t.Context(), 0, 0x0c); err == nil {
...
func TestMEMAPDoesNotModelTargetWrites(t *testing.T) {
	...
	dp := enteredDAP(t, target)
	if err := dp.WriteAP(t.Context(), 0, 0x0c, wordData); err == nil {
