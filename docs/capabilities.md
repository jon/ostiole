...
| Block reads | Yes | Accepts empty, unaligned, and mixed-width ranges. No auto-incrementing word run crosses a 1 KiB TAR boundary. Bounded posted-read pipelines return only a definitely obtained prefix on error; unread destination bytes remain untouched. |
