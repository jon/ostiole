...
| Selected AP access | Yes | `NewAPSel` constructs an explicit selector whose zero value is invalid. Access uses cached bank selection, posted reads through RDBUFF, and completed writes. Requires a connected port with no cleanup pending. |
| Ordered transactions | Yes | A single-use queue validates every DP/AP operation before traffic and settles any earlier immediate DP write before the queue runs. It settles queued DP writes and AP operations through RDBUFF, resolves every result, retains a confirmed prefix after failure, and distinguishes an untouched suffix from an indeterminate operation. FTDI HIL has read DPIDR, AP IDR, and AP CSW through the one-request-at-a-time executor. |
