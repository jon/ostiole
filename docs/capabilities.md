...
these chain operations. DAP also supports baseline JTAG-DP register access.
...
| Connection identity | Yes | `Connect` returns an opaque `Identity`; `DPIDR()` and `IDCODE()` distinguish present identification registers. The SW-DP path supplies only DPIDR. The last successful identity remains cached after release or cleanup failure. |
...
| Baseline JTAG-DP registers | Yes | Bind an explicit chain and TAP index with `dap.JTAGDP`. Four- and eight-bit IRs support IDCODE, CTRL/STAT, readable SELECT, RDBUFF, and baseline ABORT. Connection setup owns power and temporarily disables inherited ORUNDETECT; release restores its changes and returns the chain to BYPASS/Idle. Exact chain revalidation precedes restoration after scan-state loss. AP, transaction, and MEM-AP access require SWD. |
| Later JTAG-DP registers and ADIv6 | No | JTAG implements the original register set without banked registers or version detection. |
