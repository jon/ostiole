...
| DP register access | Yes | Raw methods use the currently selected bank. Explicitly banked ADIv5 methods preserve known AP fields in SELECT. On DPv3, they reject nonzero bank access. They also reject reserved banks, unavailable or read-only registers, and writes requiring unsupported SWD framing before traffic. The previous SELECT value remains available until later traffic establishes whether new write data took effect. Only a bank-zero read at `0x04` is the non-stallable CTRL/STAT access. Raw DAPABORT writes invalidate AP-derived state. Access is blocked while cleanup is pending. |
...
| Ordered transactions | Yes | A single-use queue validates every DP/AP operation before traffic and settles any earlier raw DP write before the queue runs. It settles queued DP writes and AP operations through RDBUFF, resolves every result, retains a confirmed prefix after failure, and distinguishes an untouched suffix from an indeterminate operation. FTDI HIL has read DPIDR, AP IDR, and AP CSW through the one-request-at-a-time executor. |
