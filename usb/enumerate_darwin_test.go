...
func TestDarwinListFiltersAndSortsAttachments(t *testing.T) {
	inventory := &fakeDarwinInventory{attachments: []darwinAttachment{
		{vid: 0x0403, pid: 0x6014, location: 0x02123456, address: 9, product: "FT232H", serial: "FT1234"},
	...
	want := []DeviceInfo{
		...
		{VID: 0x0403, PID: 0x6014, Bus: 2, Address: 9, Product: "FT232H", Serial: "FT1234"},
	}
...
func TestDarwinListAllUSBDevices(t *testing.T) {
	inventory := &fakeDarwinInventory{attachments: []darwinAttachment{
		{vid: 0x0d28, pid: 0x0204, location: 0x02123456, address: 9, product: "DAPLink CMSIS-DAP"},
		{vid: 0x0403, pid: 0x6014, location: 0x01123456, address: 3},
	}}
	bus := newDarwinEnumerator(inventory)

	got, err := bus.List(context.Background(), []DeviceFilter{AllDevices()})
	if err != nil {
		t.Fatalf("List: %v", err)
	}
	want := []DeviceInfo{
		{VID: 0x0403, PID: 0x6014, Bus: 1, Address: 3},
		{VID: 0x0d28, PID: 0x0204, Bus: 2, Address: 9, Product: "DAPLink CMSIS-DAP"},
	}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("List = %#v, want %#v", got, want)
	}
