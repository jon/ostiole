...
// use of the underlying connection.
...
func NewChain(conn *Conn, layout Layout) (*Chain, error) {
	...
	c := &Chain{conn: conn, layout: append(Layout(nil), layout...)}
...
// capture boundary, then parks all TAPs in BYPASS/Idle. It never changes
// board-specific chain routing.
...
// Release parks a validated chain in BYPASS/Idle and invalidates validation.
// It does not restore inherited instructions or close the wire. If state
